 ...
 *
 * Usage:
 *   pnpm tsx scripts/setup-oauth-clients.ts
 */
...
async function setupOAuthClients() {
  ...
  for (const client of OAUTH_CLIENTS) {
    ...
    try {
      ...
      const { data: existingClients, error: listError } = await supabase.auth.admin.listOAuthClients();
      ...
      const exists = existingClients?.some((c: any) => c.id === client.id || c.client_id === client.id);
      ...
      // Create OAuth client
      const { data, error } = await supabase.auth.admin.createOAuthClient({
        id: client.id,
        name: client.name,
