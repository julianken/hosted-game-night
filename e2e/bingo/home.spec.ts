...
test.describe('Bingo Home Page', () => {
  ...
  test('footer mentions Beak Gaming Platform', async ({ page }) => {
    await expect(page.getByText(/beak gaming platform/i)).toBeVisible();
  });

  test('has senior-friendly button sizes (min 44x44px)', async ({ page }) => {
    const playButton = page.getByRole('link', { name: /play now/i });
