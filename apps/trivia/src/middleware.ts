...
export async function middleware(request: NextRequest) {
  ...
  const accessToken = request.cookies.get('trivia_access_token')?.value;
  ...
  if (!isValid) {
    ...
    response.cookies.delete('trivia_access_token');
    response.cookies.delete('trivia_refresh_token');
    response.cookies.delete('trivia_user_id');
    return response;
  }
